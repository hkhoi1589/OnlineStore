...
const getQuarterlySalesReport = async () => {
	...
	const quarterlySales = out.rows.map(
		...
		(value) => ({
			label: value.year_of_date + ' - Q' + value.quarter,
			value: value.number_of_sales,
		})
	);
...
const getSalesReport = async () => {
	...
	const out = await connection.query(query);
	const sales = out.rows.map((value) => dateDataField(value.date, value.number_of_sales));
	return sales;
};
...
const getProductOrderedCountReport = async (productId) => {
	const query = `select date(orderdata.order_date) as order_day, 
                    ...
                    order by order_day`;
	const out = await connection.query(query, [productId]);
	const productOrders = out.rows.map((value) => dateDataField(value.order_day, value.sells));
...
const getOrderReport = async () => {
	const query = `select date(order_date) as date ,
                    ...
                    order by date`;
	const out = await connection.query(query);
	const productOrders = out.rows.map((value) => dateDataField(value.date, value.order_count));
	return productOrders;
};
...
const reportViewPermissionChecker = async (customerID) => {
	try {
